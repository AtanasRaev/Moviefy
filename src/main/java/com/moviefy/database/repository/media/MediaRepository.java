...
public interface MediaRepository extends JpaRepository<Movie, Long> {
    ...
    @Query(
            value = """
                    WITH movie_ids AS (
                        SELECT DISTINCT m.id
                        FROM movies m
                        JOIN cast_movies cm ON cm.movie_id = m.id
                        WHERE cm.cast_id = :castId
                    ),
                    movie_stats AS (
                        SELECT
                            AVG(m.vote_average) AS C,
                    ...
                    series_ids AS (
                        SELECT DISTINCT tv.id
                        FROM tv_series tv
                        JOIN cast_tv cs ON cs.tv_series_id = tv.id
                        WHERE cs.cast_id = :castId
                    ),
                    series_stats AS (
                        SELECT
                            AVG(tv.vote_average) AS C,
    ...
    Page<MediaProjection> findTopRatedMediaByCastId(@Param("castId") long castId, Pageable pageable);
    ...
    @Query(
            value = """
                    WITH movie_ids AS (
                        SELECT DISTINCT m.id
                        FROM movies m
                        JOIN crew_movies cm ON cm.movie_id = m.id
                    ...
                    series_ids AS (
                        SELECT DISTINCT tv.id
                        FROM tv_series tv
                        JOIN crew_tv cs ON cs.tv_series_id = tv.id
                        WHERE cs.crew_id = :crewId
                    ),
                    series_stats AS (
                        SELECT
                            AVG(tv.vote_average) AS C,
    ...
    Page<MediaProjection> findTopRatedMediaByCrewId(@Param("crewId") long crewId, Pageable pageable);
    ...
    Page<MediaProjection> findTopRatedMediaByProductionCompanyId(@Param("productionId") long id, Pageable pageable);
}
